...
function(current_dirsgs, current_tile, current_polygon){
  ...
  x_test<-st_as_sf(x=current_dirsgs[,"x1"],y=current_dirsgs[,"y1"],coords = c("x","y"))
  y_test<-st_as_sf(x=current_tile$x,y=current_tile$y, coords=c("x","y"))
  vert_outside1 <- which(sf::st_within(x_test,y_test) == 0)
  
  x_test1<-st_as_sf(x=current_dirsgs[,"x2"],y=current_dirsgs[,"y2"],coords = c("x","y"))
  y_test1<-st_as_sf(x=current_tile$x,y=current_tile$y, coords=c("x","y"))
  vert_outside2 <- which(sf::st_within(x_test1,y_test1) == 0)
  ...
  vert_outside <- sf::st_intersection(vert_outside1,vert_outside2)
  ...
  if (length(vert_outside) > 0 ) {
    for (j in 1: length(vert_outside)) {
      ...
      xy_index <- sf::st_intersection(x_coord_index,y_coord_index)
  ...
  if(nrow (ptsout) != 0){
    ...
    for (i in 1: nrow(ptsout)){
      ...
      for(j in 1: (nrow(mat1) - 1)){
        ...
        x_test2<-st_as_sf(x=x[j],y=y[j],coords = c("x","y"))
        y_test2<-st_as_sf(x=offset_polygon[,1],y=offset_polygon[,2],coords = c("x","y"))
        
        
        x_test3<-st_as_sf(x=as.character(x[j]),y=as.character(y[j]),coords = c("x","y"))
        y_test3<-st_as_sf(x=offset_polygon[,1],y=offset_polygon[,2],coords = c("x","y"))
        
        
        if(
          # (nrow(splancs::pip(xy, xy.poly, out = F, bound = TRUE)) != 0) ||  obsolete code
             (sf::st_within(x_test2,y_test2) !=0) ||
             (sf::st_within(x_test3,y_test3) !=0)){
          pt_intersect[j, ] <- xy
        }
      }
