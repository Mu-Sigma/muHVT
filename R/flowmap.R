#' state_transition_plot - Create Timeseries Plot

#' @title Function for creating state transition plots from a dataframe
#' @param df: Input dataframe containing data for state transitions
#' @param sample_size: Fraction of the dataset to sample (default is 0.2)
#' @param line_plot: Boolean flag to determine whether to create a line plot (default is NULL)
...
state_transition_plot <- function(df, sample_size = NULL, line_plot = NULL, cellid_column, time_column) {
  ...
  lined_plot <- state_transitions %>%
    ...
    add_trace(
      xend = ~ lead(Timestamp), yend = ~Next_State, type = "scatter", mode = "markers",
...
#' get_transition_probability_table - Create Transition Probability Table

#' @title Function for calculating transition probabilities for each Cell.ID in a dataframe
#'
#' @param df: Input dataframe containing data for transition probabilities
#' @param cellid_column: Name of the column containing cell IDs
#' @param time_column: Name of the column containing timestamps
#'
...
#' reconcile_transition_probability - Function for Reconciliation of Transition Probability - Model Diagnostics

#' @title Function for creating transition probability heatmaps and reconciling them
#'
#' @param df: Input dataframe containing data for transition probabilities
#' @param hmap_type: Type of heatmap to generate ('with_self_state', 'without_self_state', or 'All')
#' @param cellid_column: Name of the column containing cell IDs
#' @param time_column: Name of the column containing timestamps
...
#' generate_flow_maps - Create Flow map

#' @title Function for generating flow maps and animations based on transition probabilities
#' @param hvt_model_output: Output from a Hidden Markov Model (HMM) or other modeling approach
#' @param transition_probability_df: Dataframe containing transition probability information
#' @param hvt_plot_output: Output from a Hidden Variable Transition (HVT) model
#' @param df: Input dataframe containing data for flow map generation
#' @param animation: Type of animation ('state_based', 'time_based', or 'All')
#' @param flow_map: Type of flow map ('with_self_state', 'without_self_state', 'self_state', 'probability', or 'All')
#' @param animation_speed: Speed of animation (frames per second)
#' @param threshold: Probability threshold for flow map arrows
#' @param cellid_column: Name of the column containing cell IDs
#' @param time_column: Name of the column containing timestamps
...
generate_flow_maps <- function(hvt_model_output, transition_probability_df, hvt_plot_output, df, animation = NULL, flow_map = NULL, animation_speed = NULL, threshold = NULL, cellid_column, time_column) {
  ...
  arrow_flow_map <- ggplot() +
    geom_segment(
      data = merged_df3, mapping = aes(x = x1, y = y1, xend = x1 + (x2 - x1) * 0.8 * Probability * ifelse(threshold_label == "High Probability", 1.3, 0.9), yend = y1 + (y2 - y1) * 0.8 * Probability * ifelse(threshold_label == "High Probability", 1.3, 0.9), color = Probability),
  ...
  anime_data <- anime_data %>%
    mutate(Timestamp = round(Timestamp, 3)) %>%
    arrange(Timestamp)
  ...
  arrow_anim <- ggplot(anime_df, aes(x = x1, y = y1)) +
    geom_segment(
      data = anime_df, mapping = aes(x = x1, y = y1, xend = x1 + (x2 - x1) * 0.5, yend = y1 + (y2 - y1) * 0.5, color = label),
      arrow = arrow(length = unit(0.2, "cm")), linewidth = 1
    ) +
    geom_point(data = cellID_coordinates, aes(x = x, y = y), size = 1) +
    geom_text(data = cellID_coordinates, aes(x = x, y = y, label = Cell.ID), vjust = -1, size = 3) +
