...
#' vq.results <- list()
#' vq.results <- LBGVQ(customers, quant.err = 0.2, 
#' projection.scale = 10)
#' 
#' 
...
#' 
#' vq.results <- list()
#' vq.results <- LBGVQ(customers, quant.err = 0.5, 
#' projection.scale = 10)
#' 
#' 
#' plotHVT(vq.results, line.width = c(4), color.vec = c("red"))
#' }
#' 
#' @export LBGVQ
LBGVQ <-
function (dataset, quant.err, projection.scale) {
    ...
    deldat2[[1]] <- deldir(new_rawdeldata[[1]][[1]][, 1]*scaleFactor, 
    ...
    if(nlevel < 2){
      ...
      for(i in 2: nlevel){
        ...
        for(tileNo in 1: n_par_tile[[i]]){
          ...
          deldat2[[tileNo]] <- deldir(new_rawdeldata[[i]][[tileNo]][, 1]*scaleFactor, 
