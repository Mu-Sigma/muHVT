...
#'
#' hvt.results <- list()
#' hvt.results <- HVT(USArrests, nclust = 15, depth = 3, quant.err = 0.2, 
#'                    distance_metric = "L1_Norm", error_metric = "mean",
#'                    projection.scale = 10, normalize = TRUE,
#'                    quant_method="kmeans",diagnose=TRUE)
...
if(quant_method=="kmedoids"){message(' K-Medoids: Run time for vector qunatization using K-Medoids is very high for large number of clusters.')}
# browser()
      ...
      for (j in unique(tessdata[[i]][, "Segment.Parent"])) {
        ...
        if (any(dplyr::between(j - vec / nclust, 0, 0.9999))) {
          intermediate.rawdata[[index]] <-
            cbind(points2d[[i]][rn[dplyr::between(j - vec / nclust, 0, 0.9999)], 1],
...
# browser()
      ...
      for (i in 2:nlevel) {
        ...
        for (tileIndex in 1:n_par_tile[[(i - 1)]]) {
          ...
          rawdeldati <-
            rawdeldata[[i]][intersect(which((par_tile_indices[[i]] / nclust) <= par_tile_indices[[(i - 1)]][tileIndex]),
                                      which((par_tile_indices[[(i - 1)]][tileIndex] - 1) < (par_tile_indices[[i]] / nclust)))]
          if (nrow(gidata) != 0) {
            #transformation of rawdeldata such that they are inside the parent tile and output is stored in new_rawdeldata
            transpoints <-
              DelaunayInfo(gidata, polygon_info[[(i - 1)]][[tileIndex]], rawdeldati, nclust)
        ...
        for (tileNo in 1:n_par_tile[[i]]) {
          ...
          sec_index <-
            Hmisc::ceil(par_tile_indices[[i]][tileNo] / nclust)
        ...
        for (parentIndex in 1:n_par_tile[[i]]) {
          ...
          sec_index <-
            Hmisc::ceil(par_tile_indices[[i]] / nclust)[parentIndex]
