  ...
  function (outlier_cells, hvt_results)
   {
    ...
    outlier_rows_df <- scaled_data %>% dplyr::filter(Cell.Number %in% remove_outlier_cells)
    ...
    message(paste0(c("The following cell(s) have been removed as outliers from the dataset: ", remove_outlier_cells), collapse = " "))
