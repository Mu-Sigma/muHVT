...
function(hvt.results, line.width, color.vec, pch1 = 19, centroid.size = 3,title=NULL){
  ...
  plot(del_results[[1]][[1]], wlines = "tess", lty = 1, lwd = line.width[1], xlab = " ", ylab = " ",main=title)
  ...
  if(parlevel > 1){
    for(j in 1: parlevel){
      text(0.3,(0.8-(0.1*j)), paste("Level ", j))
      segments(0.6,(0.8-(0.1*j)),0.8,(0.8-(0.1*j)),col=color.vec[j],lty=1,lwd=line.width[j])
    }
  } else {
    text(0.3,0.7, paste("Level ", parlevel))
    segments(0.6,0.7,0.8,0.7,col=color.vec[1],lty=1,lwd=line.width[1])
  }
  } else {
