...
The muHVT package is a collection of R functions to facilitate building [topology preserving maps](https://users.ics.aalto.fi/jhollmen/dippa/node9.html) for rich multivariate data analysis. Tending towards a big data preponderance, a large number of rows. A collection of R functions for this typical workflow is organized below:
...
* __`dataset`__ - A dataframe, with numeric columns that will be used for training the model.
...
* __`depth`__   - An integer indicating the number of level in the hierarchy. A depth of 1 means no hierarchy (single level), while higher values indicate multiple levels of hierarchy.
...
* __`distance_metric`__	- The distance metric can be `L1_Norm` or `L2_Norm`. `L1_Norm` is selected by default. The distance metric is used to calculate the distance between an `n` dimensional point and centroid. 
...
* __`pch1`__	- Symbol, It plots the centroids with a particular symbol such as (solid circle, bullet, filled square, filled diamond) in the tessellations (parent levels).(default = 21 i.e filled circle).
...
* __`data`__ - A dataframe containing the test dataset. The dataframe should have all the variable(features) used for training. The variables from this dataset can also be used to overlay as heatmap.

* __`hvt.results`__ - A list of hvt.results obtained from the HVT function while performing hierarchical vector quantization on training data. The list containes detailed information about the hierarchical vector quantized data along with a summary section containing no of points, Quantization Error and the centroids for each cell, as per the n_cells given to the HVT() function.
...
* __`child.level`__ - A number indicating the level for which the heat map is to be plotted (Only used if hmap.cols is not NULL) Each level represents a different level of clustering or partitioning of the data. 

* __`normalize`__ - A logical value indicating if the columns in your dataset should be normalized. Basically it is a technique that scales the values of each variable to have a mean of 0 and a standard deviation of 1.. Default value is TRUE.

* __`distance_metric`__ - It specifies the type of distance measurement used to calculate similarity or dissimilarity between data points.The distance calculation involves taking the difference between centroid_data and data_vec either by using `Euclidean` which calculates the sum of squared differences for each column and takes the square root or `Manhattan` which calculates the sum of absolute differences for each column. Euclidean is selected by default.

* __`error_metric`__ - It specifies the error metrics to be used for evaluating the performance of the model. It can be “mean” or “max”. If error_metric is set to `mean`, the code calculates the mean quantization error within each group while if the error_metric is not set to `max`, the code calculates the maximum quantization error within each group of columns. mean is selected by default.
...
```{r}
actuals_torus <- predictions_torus[["scoredPredictedData"]]
colnames(actuals_torus) <- c("Segment.Level","Segment.Parent","Segment.Child","n","Cell.ID","Quant.Error"  ,"x_act","y_act","z_act","centroidRadius", "diff","anomalyFlag")
actuals_torus$Row_no  <- as.numeric(row.names(testTorus))

merged_result = merge(x = actuals_torus, y =hvt.torus3[[3]]$summary %>% select ("x","y","z","Cell.ID") , by = "Cell.ID") %>% select ("Row_no","x_act","y_act","z_act","Cell.ID","x","y","z")
colnames(merged_result) <- c("Row_no","x_act_norm","y_act_norm","z_act_norm","Cell.ID","x_pred","y_pred","z_pred")
merged_result<-merged_result[order(merged_result$Row_no),]

merged_result$diff <- rowMeans(abs(merged_result[, c("x_act_norm","y_act_norm","z_act_norm")] - merged_result[, c("x_pred","y_pred","z_pred")]))

rownames(merged_result) <- NULL
options(scipen = 999)
merged_result %>% head(100) %>%as.data.frame() %>%Table(scroll = T, limit = 10)
```


```{r, warning=FALSE,message=FALSE,fig.cap='Figure 13: Mean Absolute Difference'}
hist(merged_result$diff, breaks = 20, col = "blue", main = "Mean Absolute Difference", xlab = "Difference",xlim = c(0,0.20), ylim = c(0,500))
...
For more detailed information on Data Compression please refer to [section 2](https://htmlpreview.github.io/?https://github.com/Somya545/muHVT/blob/master/vignettes/muHVT_vignette.html#data-compression) of this vignette.
...
For more detailed information on Data Projection please refer to [section 3](https://htmlpreview.github.io/?https://github.com/Somya545/muHVT/blob/master/vignettes/muHVT_vignette.html#data-projection) of this vignette.
...
Lets visualize the projected Sammons 2D (440 centroids) onto a plane.
...
For more detailed information on voronoi tessellation please refer to [section 4](https://htmlpreview.github.io/?https://github.com/Somya545/muHVT/blob/master/vignettes/muHVT_vignette.html#tessellations) of this vignette.
...
For more detailed information on prediction please refer to [section 5](https://htmlpreview.github.io/?https://github.com/Somya545/muHVT/blob/master/vignettes/muHVT_vignette.html#prediction) of this vignette.
...
actuals <- predictions[["scoredPredictedData"]]
colnames(actuals) <- c("Segment.Level","Segment.Parent","Segment.Child","n","Cell.ID","Quant.Error","price_act_norm","speed_act_norm","hd_act_norm","ram_act_norm","screen_act_norm","ads_act_norm","centroidRadius", "diff","anomalyFlag")
actuals$Row_no  <- as.numeric(row.names(testComputers))

merged_result = merge(x = actuals, y =hvt.results[[3]]$summary %>% select ("price","speed","hd","ram","screen","ads","Cell.ID") , by = "Cell.ID") %>% select ("Row_no","price_act_norm","speed_act_norm","hd_act_norm","ram_act_norm","screen_act_norm","ads_act_norm","Cell.ID","price","speed","hd","ram","screen","ads")

colnames(merged_result) <-c ("Row_no","price_act_norm","speed_act_norm","hd_act_norm","ram_act_norm","screen_act_norm","ads_act_norm","Cell.ID","price_pred","speed_pred","hd_pred","ram_pred","screen_pred","ads_pred")
merged_result <- merged_result[order(merged_result$Row_no),]

merged_result$diff <- rowMeans(abs(merged_result[,c("price_act_norm","speed_act_norm","hd_act_norm","ram_act_norm","screen_act_norm","ads_act_norm")] - merged_result[, c("price_pred","speed_pred","hd_pred","ram_pred","screen_pred","ads_pred")]))
...
hist(merged_result$diff, breaks = 20, col = "blue", main = "Mean Absolute Difference", xlab = "Difference",xlim = c(0,0.6), ylim = c(0,250))
```
