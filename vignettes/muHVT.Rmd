...
Table <- function(data,scroll = F){
  kable_table <- data %>% kable(escape = F) %>% kable_styling(bootstrap_options = c("striped", "hover", "responsive"))
...
summaryTable <- function(data,scroll = T,columnName='Quant.Error',value=0.2){
...
compressionSummaryTable <- function(data,scroll = T,columnName='Percent_of_cells_below_Quantization_Error_Threshold',value=80){
...
The algorithm divides the dataset recursively into cells. The $k-means$ algorithm is used by setting $k$ to say, two in order to divide the dataset into two subsets.Then, the two subsets are divided again into two subsets by setting $k$ to two to result in a total of four subsets. The recursion terminates when the cells contain single data points or a stop criterion is reached. In this case, the stop criterion is when the cell error exceeds the quantization threshold.
...
1. Compare the quantization error for each cell to quantization threshold error
...
Now we want to calculate quantization error for each cell. For the sake of simplicity, let's consider only once cell having centroid `A` and `m` data points $F_t$ for calculating quantization error.
...
$$QE  = \frac{1}{m}\sum_{i=1}^m||A-F_t||_{p}$$ 
...
In this example, we will compress this dataset by using hierarhical VQ using k-means and visualize the Voronoi tesselation plots using Sammons projection. Later on, we will overlay price variable as heatmap to generate further insights.
...
```{r train-test computer,warning=FALSE,message=FALSE,eval = T}
...
```{r pre-processing data computer,warning=FALSE,message=FALSE,eval = T}
...
* __`nlcust`__  - Numeric. An integer indicating the number of cells per hierarchy (level)
...
into further cells only if the quantization error of the cell is above quantization error threshold.
...
```{r level one computers,warning=FALSE,message=FALSE,results='asis',eval = T}
set.seed(420)
hvt.results <- list()
hvt.results <- muHVT::HVT(train_computers,nclust = 15,depth = 1,quant.err = 0.2,projection.scale = 10,normalize = T)
```
...
```{r plot level one computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 1: The Voronoi tessellation for level 1 shown for the 15 cells in the dataset ’computers’',eval = global_var}
# Voronoi tesselation plot for level one
plotHVT(hvt.results,line.width = c(1.2), color.vec = c("#326273"))
```
...
```{r summary level one computers,warning=FALSE,eval = T}
...
hvt.results2 <- muHVT::HVT(train_computers,nclust = 15,depth = 2,quant.err = 0.2,projection.scale = 10,normalize = T)
```

Let's plot the voronoi tesselation for both the levels.

```{r plot level two computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 3: The Voronoi tessellation for level 2 shown for the 225 cells in the dataset ’computers’',eval = global_var}
# Voronoi tesselation plot for level two
plotHVT(hvt.results2, line.width = c(1.2, 0.8), color.vec = c("#326273","#955595"))
```
...
Let's analyze the summary table again for `Quant.Error` and see if any of the cells in the 2nd level have quantization error below quantization error threshold. Here we will only show first 20 results.

```{r summary level two computers,warning=FALSE,eval = global_var}
summaryTable(hvt.results2[[3]][['summary']])
```
...
As it can be seen in the table above, only `9.33%` cells in the 2nd level have quantization error below threshold. Therefore, we can go one more level deeper and try to compress the data further.
...
hvtHmap(hvt.results2, train_computers, child.level = 2,hmap.cols = "quant_error", line.width = c(0.8,0.2),color.vec = c("#326273","#955595"),palette.color = 6,show.points=TRUE)
```
...
hvt.results3 <- muHVT::HVT(train_computers,nclust = 15,depth = 3,quant.err = 0.2,projection.scale = 10,normalize = T)
```

Let's plot the voronoi tesselation for all 3 levels.

```{r plot level three computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 5: The Voronoi tessellation for level 3 shown for the 1860 cells in the dataset ’computers’',eval = global_var}
# Voronoi tesselation plot for level three
plotHVT(hvt.results3,line.width = c(1.2,0.8,0.4),color.vec = c("#326273","#955595","#7A9555"))
```

Each of the 225 cells whose quantization is above threshold in level 2 will break down into 15 cells each in level 3. Hence, as it can be seen below, level 3 has 3375 rows. So it will have 3615 rows in total.

```{r summary level three computers,warning=FALSE,eval = global_var}
summaryTable(hvt.results3[[3]][['summary']],scroll = T)
```

Let's check the compression summary to check how many cells in each level have hit the quantization error threshold

```{r compression summary level three computers,warning=FALSE,eval = T}
compressionSummaryTable(hvt.results3[[3]]$compression_summary)
```


As it can be seen from the heatmap/table summary above that the quantization error for most of the cells in level 3 fall below quantization threshold. Hence we were succesfully able to compress `84.6%` of the data.

```{r hmp level three quantization computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 6: The Voronoi tessellation with the heat map overlaid for variable ’quant_error’ in the ’computers’ dataset',eval = T}
hvtHmap(hvt.results3, train_computers, child.level = 3,hmap.cols = "quant_error", line.width = c(1.2,0.8,0.4),color.vec = c("#326273","#955595","#7A9555"),palette.color = 6)
```
...
The heatmap for `price` variable for different cells can be seen in the plot below.

```{r hmp level one computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 7: The Voronoi tessellation with the heat map overlaid for variable ’price’ in the ’computers’ dataset',eval = global_var}
...
```{r hmp level two computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 8: The Voronoi tessellation with the heat map overlaid for the variable ’price’ in the ’computer’ dataset',eval = global_var}
hvtHmap(hvt.results2, computers, child.level = 2, hmap.cols = "price",line.width = c(0.8,0.2),color.vec = c("#326273","#955595"),palette.color = 6,show.points=TRUE)
```
...
* __`hvt.results`__ - A list of hvt.results obtained from HVT function while performing hierarchical vector quantization on train data
...
set.seed(420)
predictions <- muHVT::predictHVT(test_computers,hvt.results3,hmap.cols = NULL,child.level = 3)
```
...
Table(predictions$predictions,scroll = T)
```
...
Here we will generate a 3D torus with 10000 points.
...
```{r torus hvt first,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 9: The Voronoi tessellation for level 1 shown for the 100 cells in the dataset ’torus’',eval = global_var}
set.seed(420)
hvt.torus <- muHVT::HVT(torus_df,nclust = 100,depth = 1,quant.err = 0.1,projection.scale = 10,normalize = T)
plotHVT(hvt.torus,line.width = c(1.2),color.vec = c("#326273"))
```
...
```{r torus hvt second,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 10: The Voronoi tessellation for level 1 shown for the 200 cells in the dataset ’torus’',eval = global_var}
set.seed(420)
hvt.torus2 <- muHVT::HVT(torus_df,nclust = 200,depth = 1,quant.err = 0.1,projection.scale = 10,normalize = T)
plotHVT(hvt.torus2,line.width = c(1.2),color.vec=c("#326273"))
```
...
It can be observed from the table above that only 20 cells out to 200 i.e. 10% of the cells hit the quantization threshold error.

Let's increase the number of cells to 500.

```{r torus hvt third,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 11: The Voronoi tessellation for level 1 shown for the 500 cells in the dataset ’torus’',eval = global_var}
set.seed(420)
hvt.torus3 <- muHVT::HVT(torus_df,nclust = 500,depth = 1,quant.err = 0.1,projection.scale = 10,normalize = T)
plotHVT(hvt.torus3,line.width = c(1.2),color.vec = c("#326273"))
```
...
By increasing the number of cells to 500, we were successfully able to compress `72.2%' of the data.

Can we do better?

Let's use our hierarchical vector quantization technique and go one level deeper. We will keep the number of cells as 30.

```{r torus hvt forth,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 12: The Voronoi tessellation for level 2 shown for the 510 cells in the dataset ’torus’',eval = global_var}
set.seed(420)
hvt.torus4 <- muHVT::HVT(torus_df,nclust = 30,depth = 2,quant.err = 0.1,projection.scale = 10,normalize = T)
plotHVT(hvt.torus4,line.width = c(1.2,0.8),color.vec = c("#326273","#955595"))
```
...
We can observe from the above table that we were able to compress `93.75%` of the data using hierarchical vector quantization

Now let's repeat the same process with 3D  hollow sphere
...
plot_sphere <- plot_ly(sphere_df, x= ~x, y= ~y, z = ~z, color = ~z) %>% add_markers()
plot_sphere
```
...
```{r sphere hvt first,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 13: The Voronoi tessellation for level 1 shown for the 100 cells in the dataset ’sphere’',eval = global_var}
set.seed(420)
hvt.sphere <- muHVT::HVT(sphere_df,nclust = 100,depth = 1,quant.err = 0.1,projection.scale = 10,normalize = T)
plotHVT(hvt.sphere,line.width = c(1.2),color.vec = c("#326273"))
```

Let's checkout the compression summary for sphere

```{r compression summary sphere first,warning=FALSE,eval = global_var}
...
```{r sphere hvt second,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 14: The Voronoi tessellation for level 1 shown for the 200 cells in the dataset ’sphere’',eval = global_var}
set.seed(420)
hvt.sphere2 <- muHVT::HVT(sphere_df,nclust = 200,depth = 1,quant.err = 0.1,projection.scale = 10,normalize = T)
plotHVT(hvt.sphere2,line.width = c(1.2),color.vec = c("#326273"))
```

Let's checkout the compression summary for sphere

```{r compression summary sphere two,warning=FALSE,eval = global_var}
compressionSummaryTable(hvt.sphere2[[3]]$compression_summary)
```

The table above shows that `9.5%` cells were able to hit the quantization error threshold.

Now let's increase the number of cells to 500.

```{r sphere hvt third,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 14: The Voronoi tessellation for level 1 shown for the 500 cells in the dataset ’sphere’',eval = global_var}
set.seed(420)
hvt.sphere3 <- muHVT::HVT(sphere_df,nclust = 500,depth = 1,quant.err = 0.1,projection.scale = 10,normalize = T)
plotHVT(hvt.sphere3,line.width = c(1.2),color.vec = c("#326273"))
```

Let's checkout the compression summary for sphere

```{r compression summary sphere three,warning=FALSE,eval = global_var}
...
```{r sphere hvt forth,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 15: The Voronoi tessellation for level 2 shown for the 510 cells in the dataset ’torus’',eval = global_var}
set.seed(420)
hvt.sphere4 <- muHVT::HVT(sphere_df,nclust = 30,depth = 2,quant.err = 0.1,projection.scale = 10,normalize = T)
plotHVT(hvt.sphere4,line.width = c(1.2,0.8),color.vec = c("#326273","#955595"))
```

Let's checkout the compression summary for torus

```{r compression summary sphere forth,warning=FALSE,eval = global_var}
compressionSummaryTable(hvt.sphere4[[3]]$compression_summary)
```

In this case also, `97.3%' of the cells hit the quantization threshold error.
