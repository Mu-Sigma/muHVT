...
compressionSummaryTable <- function(data,scroll = T,columnName='Percent_of_cells_below_Quantization_Error_Threshold',value=0.8){
...
set.seed(240)
hvt.results <- list()
hvt.results <- muHVT::HVT(train_computers,
                          nclust = 15,
                          depth = 1,
                          quant.err = 0.2,
                          projection.scale = 10,
...
hvt.results2 <- muHVT::HVT(train_computers,
                           nclust = 15,
                           depth = 2,
                           quant.err = 0.2,
                           projection.scale = 10,
...
Let's analyze the summary table again for `Quant.Error` and see if any of the cells in the 2nd level have quantization error below quantization error threshold. In the table below, the values for `Quant.Error` of the cells which have hit the quantization error threshold are shown in red.
...
hvt.results3 <- muHVT::HVT(train_computers,
                           nclust = 15,
                           depth = 3,
                           quant.err = 0.2,
                           projection.scale = 10,
...
As it can be seen from the heatmap/table summary above that the quantization error for most of the cells in level 3 fall below quantization threshold. Hence we were succesfully able to compress `89%` of the data.
...
The heatmap for `speed` variable for different cells can be seen in the plot below.
...
Now we will plot the heatmaps for `screen` variable

The heatmap for `screen` variable for different cells can be seen in the plot below.

```{r hmp level one computers screen,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 14: The Voronoi tessellation with the heat map overlaid for variable ’screen’ at level 1 from ’computers’ dataset',eval = global_var}
hvtHmap(hvt.results, 
        train_computers, 
        child.level = 1,
        hmap.cols = "screen", 
...
Now we will go one level deeper and overlay heatmap for `screen` at level 2

```{r hmp level two computers screen,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 15: The Voronoi tessellation with the heat map overlaid for the variable ’screen’ at level 2 from the ’computer’ dataset',eval = global_var}
hvtHmap(hvt.results2, 
        computers, 
        child.level = 2, 
        hmap.cols = "screen",
...
Let us go one level deeper and overlay heatmap for `screen` at level 3. 

In the plot below, we have overlayed heatmap for variable `screen` on level 3. 

```{r hmp level three computers screen,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 16: The Voronoi tessellation with the heat map overlaid for the variable ’screen’ at level 3 from the ’computer’ dataset',eval = global_var}
hvtHmap(hvt.results3, 
        computers, 
        child.level = 3, 
        hmap.cols = "screen",
...
set.seed(240)
predictions <- muHVT::predictHVT(test_computers,
                                 hvt.results3,
                                 hmap.cols = NULL,
                                 child.level = 3)
```
...
Let's see what cell and which level do each point belongs to.
...
set.seed(240)
test_computers <- test_computers %>% select(-c(screen,ads))
predictions <- muHVT::predictHVT(test_computers,
                                 hvt.results3,
                                 hmap.cols = NULL,
                                 child.level = 3)
...
```{r torus plot,warning=FALSE,message=FALSE,results="asis",fig.show="hold",fig.cap='Figure 17: 3D Torus',eval = global_var}
...
```{r torus hvt first,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 18: The Voronoi tessellation for level 1 shown for the 100 cells in the dataset ’torus’',eval = global_var}
set.seed(240)
...
plotHVT(hvt.torus,line.width = c(0.8),color.vec = c("#141B41"))
```
...
```{r hmp level one quantization torus,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 19: The Voronoi tessellation for level 1 with the heat map overlaid for variable ’quant_error’ in the ’torus’ dataset',eval = global_var}
hvtHmap(hvt.torus, 
        torus_df, 
        child.level = 1,
        hmap.cols = "quant_error", 
...
```{r torus hvt second,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 20: The Voronoi tessellation for level 1 shown for the 200 cells in the dataset ’torus’',eval = global_var}
set.seed(240)
hvt.torus2 <- muHVT::HVT(torus_df,
                         ...
                         projection.scale = 10,
                         normalize = T)
plotHVT(hvt.torus2,line.width = c(0.8),color.vec=c("#141B41"))
```
...
```{r hmp level two quantization torus,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 21: The Voronoi tessellation for level 2 with the heat map overlaid for variable ’quant_error’ in the ’torus’ dataset',eval = global_var}
hvtHmap(hvt.torus2, 
        train_computers, 
        child.level = 1,
        hmap.cols = "quant_error", 
...
```{r torus hvt third,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 22: The Voronoi tessellation for level 1 shown for the 500 cells in the dataset ’torus’',eval = global_var}
set.seed(240)
...
plotHVT(hvt.torus3,line.width = c(0.8),color.vec = c("#141B41"))
```
...
```{r hmp level three quantization torus,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 23: The Voronoi tessellation with the heat map overlaid for variable ’quant_error’ in the ’torus’ dataset',eval = global_var}
hvtHmap(hvt.torus3, 
        torus_df, 
        child.level = 1,
        hmap.cols = "quant_error", 
...
```{r torus hvt forth,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 24: The Voronoi tessellation for level 2 shown for the 400 cells in the dataset ’torus’',eval = global_var}
set.seed(240)
...
plotHVT(hvt.torus4,line.width = c(0.8,0.3),color.vec = c("#141B41","#0582CA"))
```
...
We can observe from the above table that we were able to compress `56%` of the data using hierarchical vector quantization

Here also we will observe the quantization error heatmap

```{r hmp level forth quantization torus,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 25: The Voronoi tessellation with the heat map overlaid for variable ’quant_error’ in the ’torus’ dataset',eval = global_var}
hvtHmap(hvt.torus4, 
        torus_df, 
        child.level = 1,
        hmap.cols = "quant_error", 
...
```{r sphere plot,warning=FALSE,message=FALSE,results="asis",fig.cap='Figure 26: 3D hollow sphere',eval = global_var}
...
```{r sphere hvt first,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 27: The Voronoi tessellation for level 1 shown for the 100 cells in the dataset ’sphere’',eval = global_var}
set.seed(240)
...
plotHVT(hvt.sphere,line.width = c(0.8),color.vec = c("#141B41"))
```
...
```{r hmp level one quantization sphere,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 28: The Voronoi tessellation for level 1 with the heat map overlaid for variable ’quant_error’ in the ’sphere’ dataset',eval = global_var}
hvtHmap(hvt.sphere, 
        sphere_df, 
        child.level = 1,
        hmap.cols = "quant_error", 
...
```{r sphere hvt second,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 29: The Voronoi tessellation for level 1 shown for the 200 cells in the dataset ’sphere’',eval = global_var}
set.seed(240)
...
plotHVT(hvt.sphere2,line.width = c(0.8),color.vec = c("#141B41"))
```
...
```{r hmp level two quantization sphere,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 30: The Voronoi tessellation for level 1 with the heat map overlaid for variable ’quant_error’ in the ’sphere’ dataset',eval = global_var}
hvtHmap(hvt.sphere2, 
        sphere_df, 
        child.level = 1,
        hmap.cols = "quant_error", 
...
```{r sphere hvt third,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 31: The Voronoi tessellation for level 1 shown for the 500 cells in the dataset ’sphere’',eval = global_var}
set.seed(240)
...
plotHVT(hvt.sphere3,line.width = c(0.8),color.vec = c("#141B41"))
```
...
```{r hmp level three quantization sphere,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 32: The Voronoi tessellation with the heat map overlaid for variable ’quant_error’ in the ’sphere’ dataset',eval = global_var}
hvtHmap(hvt.sphere3, 
        sphere_df, 
        child.level = 1,
        hmap.cols = "quant_error", 
...
```{r sphere hvt forth,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 33: The Voronoi tessellation for level 2 shown for the 576 cells in the dataset ’torus’',eval = global_var}
set.seed(240)
...
plotHVT(hvt.sphere4,line.width = c(0.8,0.3),color.vec = c("#141B41","#0582CA"))
```
...
```{r hmp level forth quantization sphere,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 34: The Voronoi tessellation for level 2 with the heat map overlaid for variable ’quant_error’ in the ’sphere’ dataset',eval = global_var}
hvtHmap(hvt.sphere4, 
        sphere_df, 
        child.level = 2,
        hmap.cols = "quant_error",
