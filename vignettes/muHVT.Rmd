...
An example of 2dimensiona VQ is shown above

In the above image, we can see that there are 5 clusters/cells and each clusters contain certain number of points. The centroid for each cluster is shown in blue. We can also call the centroids as codevectors as they represent all the points in that cluster. The set of all codevectors is called a codebook.

Now we want to calculate quantization error for each cluster. For the sake of simplicity, let's consider only cluster for calculating quantization error.
...
All the columns after this will contains centroids for each cluster for all the variables. They can be also called as 
