...
compressionSummaryTable <- function(data,scroll =
TRUE,columnName='percentOfCellsBelowQuantizationErrorThreshold',value=0.8){
summaryTable <- data %>%  dplyr::mutate_if(is.numeric, funs(round(.,2))) %>% 
dplyr::mutate(!!columnName:=  cell_spec(eval(parse(text = columnName)),
...
quantile_outlier_plots_fn <- function(data, outlier_check_var, data_cat = data[, cat_cols], 
...
upper_threshold <- stats::quantile(data[,outlier_check_var],.75,na.rm = TRUE) +
                   1.5*(stats::IQR(data[,outlier_check_var],na.rm = TRUE))
data$QuantileOutlier <- data[,outlier_check_var] > upper_threshold | data[,outlier_check_var] <
                          lower_threshold
...
ggplot2::ylab(outlier_check_var) + ggplot2::xlab("")
data <- cbind(data[, !names(data) %in% 
c("QuantileOutlier")] %>% round(2), outlier = data[, c("QuantileOutlier")])
...
summaryTable(torus_mapA[[3]]$summary,scroll = TRUE,limit = 100)
```
...
plotHVT <-(hvt.results, line.width, color.vec, pch1 = 21, centroid.size = 1.5, 
           title = NULL, maxDepth = NULL, dataset, child.level, hmap.cols,
           previous_level_heatmap = TRUE,asp = 1, ask = TRUE, tess.label = NULL, 
...
```{r,message=FALSE,warning=FALSE}
summaryTable(torus_mapC[[3]]$summary,scroll = TRUE,limit = 100)
...
NOTE: The argument 'quant.error.hmap' is set to 0.1 which is the quantization error threshold set for training the model. This parameter helps us to visualize the data with colour scheme. 

* Because in the above model training the depth is set to 2 and the n_cells is set to 30 which means the layer 1 or the first hierarchy  is split into 30 cells but as we can see the 'noOfCellsBelowQuantizationError' (from the compressionSummaryTable) is zero which means none of the 30 cells have reached the quantization error threshold.

* But in the layer 2 or the second hierarchy each of the 30 cells have the capacity to divide further into 30 cells until it reaches the quantization error threshold. so out of 900 possible cells, 898 cells(from the compressionSummaryTable ) have reached the threshold and further can't be subdivided. 

* Hence specifying the argument 'quant.error.hmap' with 0.1 helps us to visualize the heatmap of the layer where the cells have reached its quantization error threshold which is layer 2 in this case. 

* If we not specify this argument with the threshold value we get a result plot with colour schemed or heatmap plot. example is given below with the same parameters except 'quant.error.hmap' argument.

```{r hvt_mapC hmp level two z torus,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 15: The Voronoi Tessellation without the `quant.error.hmap` argument' ,eval = global_var}
...
-   **`map A`** - Result obtained from trainHVT function while performing hierarchical vector quantization on train data. This list containes information about the hierarchical vector quantized data along with a summary section.

-   **`map B`** - Result obtained from trainHVT function while performing hierarchical vector quantization on novelty data. Novelty data is a subset of the training data obtained as a result of removeNovelty function (1st element).

-   **`map C`** - Result obtained from trainHVT function while performing hierarchical vector quantization on data without novelty. This data is a subset of the training data obtained as a result of removeNovelty function (2nd element).
