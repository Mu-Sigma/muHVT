...
1) *Variable Histograms*: The distribution of all the variables in the dataset.
...
It uses an inbuilt function called `dataPlots` to display the above mentioned four objects.
...
```{r torus test summary, warning=FALSE,message=FALSE}
dataPlots(torus_test)
```
...
```{r, computers dataset distribution, warning=FALSE,message=FALSE}
dataPlots(computers)
```
