...
## 6.1 Importing Code Modules

In this section, we will load all the function scripts to the session environment.
...
In this section, we are defining all the functions to be used further in this notebook.
```{r}

###############function to generate summary table################# 
summaryTable <- function(data,scroll = TRUE,columnName='Quant.Error',value= 0.2,limit=NULL){ 
summaryTable <- data %>%  dplyr::mutate_if(is.numeric, funs(round(.,2))) %>%
dplyr::mutate(!!columnName:=  cell_spec(eval(parse(text = columnName)),
color = ifelse(is.na(eval(parse(text = columnName))),"black",
ifelse(eval(parse(text = columnName)) > value,"red","black")))) 
return(Table(summaryTable,scroll = scroll,limit = limit))  
}

################function to generate compression summary table###############
compressionSummaryTable <- function(data,scroll = TRUE,columnName='percentOfCellsBelowQuantizationErrorThreshold',value=0.8){ 
summaryTable <- data %>%  dplyr::mutate_if(is.numeric, funs(round(.,2))) %>% dplyr::mutate(!!columnName:=  cell_spec(eval(parse(text = columnName)),
color = ifelse(is.na(eval(parse(text = columnName))),"green",
ifelse(eval(parse(text = columnName)) > value,"green","black")))) 
return(Table(summaryTable,scroll = scroll))  
}

#################function to calculate summary eda###############
...
###########function to generate the box plots#############
...
correlation_plot <- function(df) {
  corrplot::corrplot(cor(df), method = "color", outline = TRUE, addgrid.col = "darkgray", addrect = 4,
                     rect.col = "black", rect.lwd = 5, cl.pos = "b", tl.col = "black", tl.cex = 1, cl.cex = 1,
                     addCoef.col = "black", number.digits = 2, number.cex = 1.5, type = "lower",
                     col = grDevices::colorRampPalette(c("maroon", "white", "darkblue"))(200))
}
...
box_plots <- lapply(names(torus_df), function(x) {
quantile_outlier_plots_fn(data = torus_df, outlier_check_var = x)[[1]]})
...
```{r Correlation, out.width="auto", out.height="480px", include=TRUE}
correlation_plot(torus_df)
```
...
The output of trainHVT function (list of 6 elements) have been explained below with an image attached for clear understanding.
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 6: Sammons 1D x Cell ID plot for layer 1 shown for the 900 cells in the torus training dataset',out.width="100%", out.height="60%"}
plotHVT(plot.type = '1D')
```
...
```{r, warning=FALSE,message=FALSE, fig.cap='Figure 7: Sammons 2D Plot for 100 cells'}
...
```{r, message=FALSE,warning=FALSE,fig.cap='Figure 8: Sammons 2D Plot for 300 cells'}
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 9: Sammons 2D Plot for 900 cells'}
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 10: The Voronoi tessellation for layer 1 shown for the 100 cells in the torus training dataset'}
plotHVT(
  hvt.torus,
  line.width = c(0.4),
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 11: The Voronoi tessellation for layer 1 shown for the 300 cells in the torus training dataset'}
plotHVT(
  hvt.torus2,
  line.width = c(0.4),
  color.vec = c("navy blue"),
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 12: The Voronoi tessellation for layer 1 shown for the 900 cells in the torus training dataset'}
plotHVT(
  hvt.torus3,
  line.width = c(0.4),
  color.vec = c("navy blue"),
...
```{r hmp level two quantization torus n demo,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 13: The Voronoi tessellation for layer 1 and number of cells 900 with the heat map overlaid for `No. of entities in each cell` in the torus dataset',eval = global_var}
plotHVT(
  hvt.torus3,
  torus_df,
  child.level = 1,
...
```{r hmp level two quantization torus x demo,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 14: The Voronoi tessellation for layer 1 and number of cells 900 with the heat map overlaid for variable `x` in the torus dataset',eval = global_var}
plotHVT(
  hvt.torus3,
  torus_df,
  child.level = 1,
...
```{r hmp level two quantization torus y demo,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 15: The Voronoi tessellation for layer 1 and number of cells 900 with the heat map overlaid for variable `y` in the torus dataset',eval = global_var}
plotHVT(
  hvt.torus3,
  torus_df,
  child.level = 1,
...
```{r hmp level two quantization torus z demo,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 16: The Voronoi tessellation for layer 1 and number of cells 900 with the heat map overlaid for variable `z` in the torus dataset',eval = global_var}
plotHVT(
  hvt.torus3,
  torus_df,
  child.level = 1,
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 17: Mean Absolute Difference'}
...
box_plots <- lapply(names(computers_numeric), function(x) {
quantile_outlier_plots_fn(data = computers_numeric, outlier_check_var = x)[[1]]})
do.call(gridExtra::grid.arrange, c(grobs = box_plots, ncol = 2))
...
```{r Correlation_2, fig.height=6, fig.width=6, purl=FALSE, include=TRUE}
correlation_plot(computers_numeric)
```
**Train - Test Split**
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 18: Sammons 1D x Cell ID plot for layer 1 shown for the 440 cells in the dataset ’computers’',out.width="100%", out.height="60%"}
plotHVT(plot.type = '1D')
```
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 19: Sammons 2D Plot for 440 cells'}
...
```{r plot level one computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 20: The Voronoi Tessellation for layer 1 shown for the 440 cells in the dataset ’computers’',eval = global_var}
plotHVT(hvt.results,
        line.width = c(0.2), 
        color.vec = c("navy blue"),
        centroid.size = 0.01,  
...
```{r hmp level one n computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 21: The Voronoi Tessellation with the heat map overlaid over the `No. of entities in each cell` in the ’computers’ dataset',eval = global_var}
plotHVT(
  hvt.results,
  trainComputers,
  child.level = 1,
...
```{r hmp level one price computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 22: The Voronoi Tessellation with the heat map overlaid over the variable `price` in the ’computers’ dataset',eval = global_var}
...
```{r hmp level one hd computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 23: The Voronoi Tessellation with the heat map overlaid over the variable `hd` in the ’computers’ dataset',eval = global_var}
...
```{r hmp level one ram computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 24: The Voronoi Tessellation with the heat map overlaid over the variable `ram` in the ’computers’ dataset',eval = global_var}
plotHVT(
  hvt.results,
  trainComputers,
  child.level = 1,
...
```{r hmp level one screen computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 25: The Voronoi Tessellation with the heat map overlaid over the variable `screen` in the ’computers’ dataset',eval = global_var}
plotHVT(
  hvt.results,
  trainComputers,
  child.level = 1,
...
```{r hmp level one ads computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 26: The Voronoi Tessellation with the heat map overlaid over the variable `ads` in the ’computers’ dataset',eval = global_var}
...
```{r,message=FALSE,warning=FALSE,fig.cap='Figure 27: Mean Absolute Difference'}
