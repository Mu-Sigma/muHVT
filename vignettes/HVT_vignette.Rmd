...
Here, we will generate a 3D torus (a torus is a surface of revolution generated by revolving a circle in three-dimensional space one full revolution about an axis that is coplanar with the circle) with 9600 points.
...
Let us first split the data into train and test. We will select first 80% of the data for training and remaining as testing.
...
Now, lets have a look at randomly selected **testing dataset containing(2400 data points)**.For the sake of brevity we are displaying first six rows.
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 3: Sammons 1D x Cell ID plot for layer 1 shown for the 100 cells in the dataset ’torus’'}
plotHVT(heatmap = '1D')
```
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 4: Sammons 1D x Cell ID plot for layer 1 shown for the 300 cells in the dataset ’torus’'}
plotHVT(heatmap = '1D')
```
...
**By increasing the number of cells to 900, we were successfully able to compress `84%` of the data, so we will not further subdivide the cells**
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 5: Sammons 1D x Cell ID plot for layer 1 shown for the 900 cells in the dataset ’torus’'}
plotHVT(heatmap = '1D')
```
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 9: The Voronoi tessellation for layer 1 shown for the 100 cells in the dataset ’torus’'}
plotHVT(
  hvt.torus,
  line.width = c(0.4),
  color.vec = c("#141B41"),
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 10: The Voronoi tessellation for layer 1 shown for the 300 cells in the dataset ’torus’'}
plotHVT(
  hvt.torus2,
  line.width = c(0.4),
  color.vec = c("#141B41"),
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 11: The Voronoi tessellation for layer 1 shown for the 900 cells in the dataset ’torus’'}
plotHVT(
  hvt.torus3,
  line.width = c(0.4),
  color.vec = c("#141B41"),
...
Let's see which cell and level each point belongs to and check the mean absolute difference for each of the **2400 records**. For the sake of brevity, we will only show the first 10 rows
...
Act_pred_Table %>% head(100) %>%as.data.frame() %>%Table(scroll = TRUE, limit = 10)
```
...
# Quick peek
Table(head(computers), scroll = TRUE, limit = 20)
```
...
Now, lets have a look at the randomly selected **testing dataset** containing (1252 data points). For the sake of brevity we are displaying first six rows.
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 17: Sammons 1D x Cell ID plot for layer 1 shown for the 440 cells in the dataset ’computers’'}
plotHVT(heatmap = '1D')
```
...
Now, lets have a look at the randomly selected **testing dataset** containing (1252 data points) before we pass it to scoreHVT function for scoring. For the sake of brevity we are displaying first six rows.
...
Now once we have built the model, let us try to score using our test dataset containing(1252 data points) which cell and which level each point belongs to.
