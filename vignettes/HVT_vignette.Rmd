...
* __`cell_id`__ - (2Dhvt) Logical. To indicate whether the plot should have Cell IDs or not. (default = FALSE)
...
* __`distance_metric`__ - The distance metric can be `L1_Norm`(Manhattan) or `L2_Norm`(Eucledian). The metric is used when calculating distance between each datapoint(in test dataset) with the centroids obtained from results of trainHVT. Default is `L1_Norm`.
