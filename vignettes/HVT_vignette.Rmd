...
We will pass the below mentioned model parameters along with torus training dataset (containing 9000 datapoints) to `trainHVT` function.
...
## Step 4: Scoring(scoreHVT)

**Raw Testing Dataset**

Lets have a look at our randomly selected test dataset containing (3000 points) before we pass it to scoreHVT function for scoring.
...
Now once we have built the model, let us try to score using our test dataset (containing 3000 data points) which cell and which level each point belongs to.
...
Now, lets have a look at the randomly selected **raw testing dataset** containing (1252 data points) before we pass it to scoreHVT function for scoring. For the sake of brevity we are displaying first six rows.
...
*  We have randomly selected 9000 datapoints for training and remaining datapoints for validation.
