...
plot_ly(x = torus_df$x, y = torus_df$y, z = torus_df$z, type = 'scatter3d',mode = 'markers',marker = list(
        color = torus_df$z,colorscale = c('#E62E00', '#0026E6'),showscale = TRUE,size = 3,colorbar = list(title = 'z'))) %>%
...
```{r}
data_table <- summary_eda(torus_df)
Table(data_table, scroll = TRUE)
```
...
torus dataset across panels. These plots will display the median and Inter
Quartile Range of each column at a panel level.

```{r, results='asis', warning = FALSE, purl = FALSE, include=FALSE}
...
cat_cols <-
  colnames(torus_df)[unlist(sapply(
    torus_df,
    FUN = function(x) {
...
quantile_outlier_plots_fn <- function(data, outlier_check_var, data_cat = torus_df[, cat_cols], numeric_cols = numeric_cols){
    # lower threshold
...
```{r Correlation, echo=FALSE, purl=FALSE, out.width="auto", out.height="480px"}

corrplot::corrplot(cor(torus_df), method = "color",outline = TRUE,addgrid.col = "darkgray",addrect = 4,
rect.col = "black",rect.lwd = 5,cl.pos = "b",tl.col = "black",tl.cex = 1,cl.cex = 1,addCoef.col = "black",
number.digits = 2,number.cex = 1.5,type = "lower",
...
Now lets have a look at **structure** and **summary** of the training data.
...
```{r}
train_table <- summary_eda(torus_train)
Table(train_table, scroll = TRUE)
```
...
Now lets have a look at **structure** and **summary** of the test data.

```{r torus test structure, warning=FALSE, eval = global_var}
str(torus_test)
```

```{r torus test summary, warning=FALSE,eval = global_var}
test_table <- summary_eda(torus_test)
Table(test_table, scroll = TRUE)
```
...
* __`hvt_validation`__ - A logical value indicating whether user wants to holdout a validation set and find mean absolute deviation of the validation points from the centroid. These values can be found in the 6th element of the result list under the model.info under the title validation_result. Default value is FALSE.
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 3: Sammons 1D x Cell ID plot for layer 1 shown for the 100 cells in the torus training dataset'}
plotHVT(heatmap = '1D')
```
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 4: Sammons 1D x Cell ID plot for layer 1 shown for the 300 cells in the torus training dataset',out.width="100%", out.height="60%"}
plotHVT(heatmap = '1D')
```
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 5: Sammons 1D x Cell ID plot for layer 1 shown for the 900 cells in the torus training dataset',out.width="100%", out.height="60%"}
plotHVT(heatmap = '1D')
```
...
Table(head(centroid_coordinates1), scroll = TRUE, limit = 20)
...
```{r, warning=FALSE,message=FALSE, fig.cap='Figure 6: Sammons 2D Plot for 100 cells'}
...
Table(head(centroid_coordinates2), scroll = TRUE, limit = 20)
...
```{r, message=FALSE,warning=FALSE,fig.cap='Figure 7: Sammons 2D Plot for 300 cells'}
...
Table(head(centroid_coordinates3), scroll = TRUE, limit = 20)
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 8: Sammons 2D Plot for 900 cells'}
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 9: The Voronoi tessellation for layer 1 shown for the 100 cells in the torus training dataset'}
plotHVT(
  hvt.torus,
  line.width = c(0.4),
  color.vec = c("#141B41"),
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 10: The Voronoi tessellation for layer 1 shown for the 300 cells in the torus training dataset'}
plotHVT(
  hvt.torus2,
  line.width = c(0.4),
  color.vec = c("#141B41"),
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 11: The Voronoi tessellation for layer 1 shown for the 900 cells in the torus training dataset'}
plotHVT(
  hvt.torus3,
  line.width = c(0.4),
  color.vec = c("#141B41"),
...
```{r hmp level two quantization torus n demo,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 12: The Voronoi tessellation for layer 1 and number of cells 900 with the heat map overlaid for `No. of entities in each cell` in the torus dataset',eval = global_var}
plotHVT(
  hvt.torus3,
  torus_df,
  child.level = 1,
...
```{r hmp level two quantization torus x demo,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 13: The Voronoi tessellation for layer 1 and number of cells 900 with the heat map overlaid for variable `x` in the torus dataset',eval = global_var}
plotHVT(
  hvt.torus3,
  torus_df,
  child.level = 1,
...
```{r hmp level two quantization torus y demo,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 14: The Voronoi tessellation for layer 1 and number of cells 900 with the heat map overlaid for variable `y` in the torus dataset',eval = global_var}
plotHVT(
  hvt.torus3,
  torus_df,
  child.level = 1,
...
```{r hmp level two quantization torus z demo,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 15: The Voronoi tessellation for layer 1 and number of cells 900 with the heat map overlaid for variable `z` in the torus dataset',eval = global_var}
plotHVT(
  hvt.torus3,
  torus_df,
  child.level = 1,
...
* __`data`__ - A dataframe containing the testing dataset. The dataframe should have all the variable(features) used for training. 
...
* __`mad.threshold`__  - A numeric value indicating the permissible Mean Absolute Deviation which is obtained from Minimum Intra centroid plot(when diagnose is set to TRUE).
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 16: Mean Absolute Difference'}
...
sum_table <- summary_eda(computers)
Table(sum_table, scroll = TRUE)
```

Now, let us check the **Data Distribution** of the computers dataset.

**Variable Histograms**

Shown below is the distribution of all the variables in the computers dataset.

NOTE: Only the continuous variable can be plotted, hence the numeric columns from the `computers` dataset is selected.

```{r, warning=FALSE,message=FALSE}
eda_cols <- names(computers)[sapply(computers, is.numeric)] # Select only numeric columns
dist_list <- lapply(1:length(eda_cols), function(i){
generateDistributionPlot(computers, eda_cols[i]) })
do.call(gridExtra::grid.arrange, args = list(grobs = dist_list, ncol = 2, top = "Distribution of Features"))
```
...
cat_cols <-
  colnames(computers)[unlist(sapply(
    computers,
    FUN = function(x) {
      CheckColumnType(x) == "character" ||
        CheckColumnType(x) == "factor" }))]
...
## the below function helps plotting quantile outlier plot for multiple variables
quantile_outlier_plots_fn <- function(data, outlier_check_var, data_cat = computers[, cat_cols], numeric_cols = numeric_cols){
    # lower threshold
    lower_threshold <- stats::quantile(data[, outlier_check_var], .25,na.rm = TRUE) - 1.5*(stats::IQR(data[, outlier_check_var], na.rm = TRUE))
...
```{r message=FALSE, warning=FALSE,results='asis', fig.height=6, fig.width=6}
computers_numeric <- computers[sapply(computers, is.numeric)]
box_plots <- list()
for (x in names(computers_numeric)) {
box_plots[[x]] <- quantile_outlier_plots_fn(data = computers_numeric, outlier_check_var = x)[[1]]}
gridExtra::grid.arrange(grobs = box_plots, ncol = 2)
...
```{r Correlation_2, echo=FALSE, fig.height=6, fig.width=6, purl=FALSE}

corrplot::corrplot(cor(computers_numeric), method = "color",outline = TRUE,addgrid.col = "darkgray",addrect = 4,
rect.col = "black",rect.lwd = 5,cl.pos = "b",tl.col = "black",tl.cex = 1,cl.cex = 1,addCoef.col = "black",
number.digits = 2,number.cex = 1.5,type = "lower",
col = grDevices::colorRampPalette(c("#982D3B","white","#235C89"))(200))
...
trainComputers <-trainComputers %>% dplyr::select(-c( cd, multi, premium, trend))
testComputers <-testComputers %>% dplyr::select(-c( cd, multi, premium, trend))
```
...
Now let's have a look at **structure** and **summary** of the training dataset.
...
```{r}
train_table_comp <- summary_eda(trainComputers_data)
Table(train_table_comp, scroll = TRUE)
```

Now, let us check the **Data Distribution** of the training dataset.
...
```{r, warning=FALSE,message=FALSE}
eda_cols <- names(trainComputers_data)
dist_list <- lapply(1:length(eda_cols), function(i){
generateDistributionPlot(trainComputers_data, eda_cols[i]) })
do.call(gridExtra::grid.arrange, args = list(grobs = dist_list, ncol = 2, top = "Distribution of Features"))
...
Now let's have a look at **structure** and **summary** of the testing dataset.

```{r computers structure, warning=FALSE, eval = global_var}
str(testComputers_data)
```

```{r computers test summary, warning=FALSE,eval = global_var}
test_table_comp <- summary_eda(testComputers_data)
Table(test_table_comp, scroll = TRUE)
```

Now, let us check the **Data Distribution** of the testing dataset.
...
```{r, warning=FALSE,message=FALSE}
eda_cols <- names(testComputers_data)
dist_list <- lapply(1:length(eda_cols), function(i){
generateDistributionPlot(testComputers_data, eda_cols[i]) })
do.call(gridExtra::grid.arrange, args = list(grobs = dist_list, ncol = 2, top = "Distribution of Features"))
```
...
summaryTable(hvt.results[[3]]$summary, scroll = TRUE, limit = 100)
```
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 17: Sammons 1D x Cell ID plot for layer 1 shown for the 440 cells in the dataset ’computers’',out.width="100%", out.height="60%"}
plotHVT(heatmap = '1D')
```
...
```{r, warning=FALSE,message=FALSE,fig.cap='Figure 18: Sammons 2D Plot for 440 cells'}
...
```{r plot level one computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 19: The Voronoi Tessellation for layer 1 shown for the 440 cells in the dataset ’computers’',eval = global_var}
plotHVT(hvt.results,
        line.width = c(0.2), 
        color.vec = c("#141B41"),
        centroid.size = 0.01,  
...
```{r hmp level one n computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 20: The Voronoi Tessellation with the heat map overlaid over the `No. of entities in each cell` in the ’computers’ dataset',eval = global_var}
plotHVT(
  hvt.results,
  trainComputers,
  child.level = 1,
...
```{r hmp level one price computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 21: The Voronoi Tessellation with the heat map overlaid over the variable `price` in the ’computers’ dataset',eval = global_var}
...
```{r hmp level one hd computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 22: The Voronoi Tessellation with the heat map overlaid over the variable `hd` in the ’computers’ dataset',eval = global_var}
...
```{r hmp level one ram computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 23: The Voronoi Tessellation with the heat map overlaid over the variable `ram` in the ’computers’ dataset',eval = global_var}
plotHVT(
  hvt.results,
  trainComputers,
  child.level = 1,
...
```{r hmp level one screen computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 24: The Voronoi Tessellation with the heat map overlaid over the variable `screen` in the ’computers’ dataset',eval = global_var}
plotHVT(
  hvt.results,
  trainComputers,
  child.level = 1,
...
```{r hmp level one ads computers,warning=FALSE,fig.show='hold',results='hide',message=FALSE,fig.cap='Figure 25: The Voronoi Tessellation with the heat map overlaid over the variable `ads` in the ’computers’ dataset',eval = global_var}
...
scoring_comp[["scoredPredictedData"]] %>% head(100) %>% round(2) %>% as.data.frame() %>%  Table(scroll = TRUE, limit = 100)
```
...
```{r,message=FALSE,warning=FALSE,fig.cap='Figure 26: Mean Absolute Difference'}
...
* Once again, we generated a compressed HVT map (hvt.torus3) using the trainHVT() algorithm on the torus dataset. The parameters for this map were set to `n_cells = 900`, `quant.error = 0.1`, and `depth = 1`. Upon analyzing the compression summary, we    found that 83% of the 100 cells have reached the quantization threshold error and we can clearly visualize the 3D torus(donut) in 2D   space.
...
*  We construct a compressed HVT map using the trainHVT() on the training dataset by setting __`n_cells`__ to 440 and  __`quant.error`__ to 0.2, and we were able to attain a compression of 81%
