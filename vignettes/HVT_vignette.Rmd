...
Let's have look at the `plotHVT` function which we will use to overlay a variable as heatmap.

```{r plotHVT function demo,echo = TRUE, eval= FALSE}
plotHVT(hvt.results, dataset, child.level, hmap.cols, color.vec ,line.width, palette.color = 6)
```
...
set.seed(240)
hvt.results <- list()
hvt.results <- HVT::trainHVT(trainComputers,   
                          n_cells = 440,
                          depth = 1,
                          quant.err = 0.2,
                          projection.scale = 10,
