...
## HVT (v24.1.1) | What's New? {-}

09th February, 2024

In this version of HVT package, the following new features have been introduced:

1. **Nomenclature:** To enhance consistency and impart the function names with greater contextual relevance, we have opted to standardize the nomenclature by employing camel case formatting throughout the entirety of the package. Given below are the few instances.
* `HVT` to `trainHVT`
* `predictHVT` to `scoreHVT`
* `predictLayerHVT` to `scoreLayeredHVT`

2. **Restructured:** The functions have been strategically reorganized and categorized under newly established sections to infuse greater significance into their purpose across the package. These sections are prominently featured on the index page of the package's PDF documentation.Given below are the few instances.
...
3. **Enhancements:** The pre-existed functions, `hvtHmap` & `exploded_hmap`, have been consolidated and added to the function `plotHVT`. Additionally, we have expanded the functionality of `plotHVT` to include 1D plotting capabilities. Users can generate various types of plots, including 1D, 2D hvt, 2D heatmap, and Interactive surface plots, all using a single, versatile function. This consolidation aims to create a robust framework to meet all plot-related requirements efficiently.
...
```{r mlayer_flow,echo=FALSE,warning=FALSE,fig.show='hold',message=FALSE,out.width='90%',fig.height=8,fig.cap='Figure 2: Data Segregation for scoring based on a sequence of maps using scoreLayeredHVT()'}
