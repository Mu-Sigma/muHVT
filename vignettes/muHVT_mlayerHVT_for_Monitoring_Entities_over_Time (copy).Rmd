...
Table(hvt_mapB, scroll = T, limit = 10)
```
...
summaryTable(cell_mapping_mapC)
```
...
Table(ground_truth_df, scroll = T, limit = 10)
```
...
predictions <- predictions[order(predictions$MapB.Cell.ID,decreasing=TRUE),]
Table(predictions, scroll = T, limit = 10)
```
