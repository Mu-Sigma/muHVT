...
list.of.packages <- c("dplyr", "kableExtra", "geozoo", "plotly", "purrr", "sp", "muHVT", "data.table", "gridExtra")
...
```{r torus plot,warning=FALSE,message=FALSE,fig.show="hold",fig.cap='Figure 2: 3D Torus',eval = F}
scaling_factor <- 0.5
library(rgl)
...
Now let's try to use the package and project the above 3D object in a 2D Space. However, let's try to comprehend the **HVT function** first before moving on. 
...
**It can be observed from the table above that only 43 cells out of 300 i.e. `14%` of the cells hit the Quantization Error threshold**
...
**By increasing the number of cells to 900, we were successfully able to compress `93%` of the data**
...
**Now lets explore a little more and try to project the torus(3D) object to 2D Space with the below mentioned model parameters at level 2**
...
**It can be observed from the table above that only 114 cells out of 400 i.e. `28%` of the cells hit the Quantization Error threshold**
...
**It can be observed from the table above that only 535 cells out of 666 i.e. `80%` of the cells hit the Quantization Error threshold**
...
### 4.2 Example Usage of Hierarchical VQ with Sammons Projection and Tessellation on Personal Computers Dataset.
...
**As it can be seen from the table above, __`83%`__ of the cells have hit the quantization threshold error**


Now let's try to understand **plotHVT** function. The parameters have been explained in detail below

```{r plotHVT function demo,echo = TRUE, eval= FALSE}
plotHVT(map_A, line.width, color.vec, pch1 = 21, centroid.size = 3, title = NULL, maxDepth = 1)
```

* __`map_A`__ -  A list containing the output of the HVT function which has the details of the tessellations to be plotted
...
hvtHmap(map_A, dataset, child.level, hmap.cols, color.vec ,line.width, palette.color = 6)
```

* __`map_A`__ - A list of results obtained from the HVT function
...
The datatable displayed below are the data with novelties.For the sake of brevity, we will only show the first 15 rows
...
**We now have the set of maps (map A, map B & map C) which will be used to predict which map and cell each test record is assigned to.**
...
6. Embedding : https://en.wikipedia.org/wiki/Word_embedding
