...
list.of.packages <<- c("DT",'plotly', "magrittr", "data.table", "tidyverse", "crosstalk", "kableExtra", "gganimate","gdata",
                       "HVT", "data.table", "jmuOutlier", "viridis", "deldir", "conf.design", "splancs","Hmisc","xfun","sp",
...
Firstly, we will load all the function scripts to the session environment.
...
```{r hist_plot, purl=TRUE, echo = FALSE}
##############function to generte histogram plots#############
...
quantile_outlier_plot <- ggplot2::ggplot(data, ggplot2::aes(x="", y = data[,outlier_check_var])) +
             ...
             ggplot2::theme_bw() + 
             ggplot2::theme(panel.border=ggplot2::element_rect(size=0.1),panel.grid.minor.x=ggplot2::element_blank(),
...
box_plots <- lapply(names(dataset_updated), function(x) {
quantile_outlier_plots_fn(data = dataset_updated, outlier_check_var = x)[[1]]})
...
```{r Correlation, fig.height=6, fig.width=6, purl=FALSE,fig.align='center'}

corrplot::corrplot(cor(dataset_updated),method = "color",outline = TRUE,addgrid.col = "darkgray", addrect = 4,
