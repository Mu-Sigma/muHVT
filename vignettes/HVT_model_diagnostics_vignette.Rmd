...
author: "Zubin Dowlaty, Shubhra Prakash, Sunuganty Achyut Raj, Praditi Shah, Somya Shambhawi"
...
summaryTable <- function(data,scroll = TRUE,columnName='Quant.Error',value=0.1,limit=NULL){
...
## 4.1.1 Import Dataset from Local
...
```{r}
data_table <- summary_eda(dataset_updated)
...
## 5.2 Deleting Irrelevant Columns
...
## 5.3 Formatting and Renaming Columns
...
## 5.4 Changing Data Type of Columns
...
## 5.5 Checking and Removing Duplicates
...
## 5.6 List of Numerical and Categorical Column Names
...
## 5.7 Filtering Dataset for Analysis
...
### 5.8.2 Check for Singularity {#singularity}
...
### 5.8.3 Selecting only Numeric Cols after Dummification
...
```{r}
final_table <- summary_eda(dataset_updated)
Table(final_table, scroll = TRUE)
```
...
```{r dist_plots_true2, purl = FALSE,message=FALSE,warning=FALSE, fig.height=6, fig.width=6, fig.align='center'}
...
```{r message=FALSE, warning=FALSE,results='asis', fig.height=6, fig.width=6,fig.align='center'}
...
```{r Correlation, echo=FALSE, fig.height=6, fig.width=6, purl=FALSE,fig.align='center'}
...
-   **`dataset`** - A dataframe with numeric columns for training the model

-   **`n_cells`** - An integer indicating the number of cells per
    hierarchy (level)

-   **`depth`** - An integer indicating the number of levels. (1 = No
    hierarchy, 2 = 2 levels, etc ...)

-   **`quant.error`** - A number indicating the quantization error
    threshold. A cell will only breakdown into further cells if the
    quantization error of the cell is above the defined quantization
    error threshold

-   **`distance_metric`** - The distance metric can be `L1_Norm(Manhattan)` or
    `L2_Norm(Euclidean)`. `L1_Norm` is selected by default. The distance metric is
    used to calculate the distance between an `n` dimensional point and
    centroid. The user can also pass a custom function to calculate this
    distance

-   **`error_metric`** - The error metric can be `mean` or `max`. `max`
    is selected by default. `max` will return the max of `m` values and
    `mean` will take mean of `m` values where each value is a distance
    between a point and centroid of the cell. Moreover, the user can
    also pass a custom function to calculate the error metric

-   **`quant_method`** - The quantization method can be `kmeans` or
    `kmedoids`. `kmeans` is selected by default

-   **`normalize`** - A logical value indicating whether the columns in
    your dataset need to be normalized. Default value is TRUE. The
    algorithm supports Z-score normalization

-   **`diagnose`** - A logical value indicating whether user wants to
    perform diagnostics on the model. Default value is FALSE.

-   **`hvt_validation`** - A logical value indicating whether user wants
...
## 7.1 Model Diagnostics and Validation

### 7.1.1 Diagnostics {.unlisted .unnumbered}
...
### 7.1.2 Validation {.unlisted .unnumbered}
...
### 7.2.1 Mean Absolute Deviation based Calibration: HVT Model \| Train Data
...
### 7.2.2 Minimum Intra-DataPoint Distance Plot: Train Data
...
### 7.2.3 Minimum Intra-Centroid Distance Plot: HVT Model \| Train Data
...
### 7.2.4 Distribution of Number of Observations in HVT Cells
...
### 8.2.1 Transformation of Categorical Features
...
- **`mad.threshold`**  - A numeric value indicating the permissible Mean Absolute Deviation which is obtained from Minimum Intra centroid plot.
...
### 8.3.1 Quantization Error Comparison (Train vs. Test)
