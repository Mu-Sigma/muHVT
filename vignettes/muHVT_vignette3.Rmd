...
This function takes several input parameters, including the test dataset (data) and the set of maps obtained as a result from `HVT` function .This function first performs predictions on the test dataset using each of the map separately. It calls the `predictHVT` function to predict the cell and level for each point in the test dataset based on the respective HVT maps. Next,  the function combines the predictions from the different HVT configurations into a single predictions table. It merges the predictions from map A, map B, and map C based on the row number of the test dataset. The resulting table contains information about the predicted cell and level from each HVT configuration.The function then processes the predictions to handle identified novelty cells. It assigns the cell ID from map B to the corresponding points if they belong to identified novelty cells in map A. If a point does not belong to a novelty cell, it assigns the cell ID from map C. 
...
# Example Usage : Visualizing Multidimensional Data with Sammon's Projection using Torus (Donut)
...
Now let's do some EDA on the data. First of all, we will see how the data looks like. For the shake of brevity we are displaying first six rows.
...
<!-- To achieve the projection of a 3D object onto a 2D space, we will follow the outlined steps provided in the abstract section of this vignette -->
...
## Refining Torus Projection at Level 2
...
for detailed information on data Compression please refer to [section 2](https://htmlpreview.github.io/?https://github.com/Somya545/muHVT/blob/master/vignettes/muHVT_vignette3.html#compress-vector-quantization) of this vignette.
...
**It can be observed from the table above that none of the cells has reached the Quantization Error threshold. Therefore we can further subdivide the cells by increasing the n_cells inorder to achieve a compression of 80%**
...
**It can be observed from the table above that only 535 cells out of 666 i.e. `80%` of the cells have reached the Quantization Error threshold. Since we have attained 80% compression we will not further subdivide the cells**
...
For detailed information on Data Projection please refer to [section 3](https://htmlpreview.github.io/?https://github.com/Somya545/muHVT/blob/master/vignettes/muHVT_vignette3.html#projection) of this vignette.
...
# Example Usage of Predictions using the predictLayerHVT on Personal Computers Dataset.
...
For more detailed information on Data Compression please refer to [section 2](https://htmlpreview.github.io/?https://github.com/Somya545/muHVT/blob/master/vignettes/muHVT_vignette3.html#compress-vector-quantization) of this vignette.
...
**As it can be seen from the table above, __`83%`__ of the cells have reached the quantization threshold error. Since we are successfully able to compress `83%` of the data, so we will not further subdivide the cells **
...
The datatable displayed below are the data with novelties. For the shake of brevity we are displaying first nine rows.
...
**It can be observed from the table above that  3 cells out of 3 i.e. __`100%`__ of the cells has reached the Quantization Error threshold**
...
**It can be observed from the table above that 829 cells out of 1001 i.e. __`83%`__ of the cells has reached the Quantization Error threshold**
...
## Prediction : predictLayerHVT

For deatiled information about the prediction please refer to [section 5](https://htmlpreview.github.io/?https://github.com/Somya545/muHVT/blob/master/vignettes/muHVT_vignette3.html#prediction) of this vignette.
