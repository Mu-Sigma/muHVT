...
* __`dataset`__ - A dataframe with numeric columns. This is the input dataset on which the compression algorithm will be applied.

* __`min_compression_perc`__  -  An integer indicating the minimum percent compression rate to be achieved for the dataset. The algorithm aims to compress the dataset while maintaining at least this specified compression percentage.
...
* __`depth`__   - An integer indicating the number of levels in the hierarchy. A depth of 1 means no hierarchy (single level), while higher values indicate multiple levels of hierarchy.
...
* __`projection.scale`__ - A number indicating the scale factor for the tesselations so as to visualize the sub-tesselations. It helps in adjusting the visual representation of the hierarchy to make the sub-tesselations more visible.
...
* __`map A`__ - A list of hvt.results.model obtained from HVT function while performing hierarchical vector quantization on train data. This list containes information about the hierarchical vector quantized data along with a summary section.
...
* __`mad.threshold`__ -  A threshold value used to filter data based on the Median Absolute Deviation (MAD) of the Quant.Error variable. It determines how extreme a deviation from the median has to be in order to be considered an novelty

* __`normalize`__ -  A logical value indicating if the columns in the dataset should be normalized. If set to TRUE, the algorithm will normalize the data before performing compression. The default value is TRUE
...
```{r}
merged_df <- new_predict[["actual_predictedTable"]] %>%
  transmute(Row.Number,act_price,act_speed,act_hd,act_ram,act_screen,act_ads,Layer1.Cell.ID,Layer2.Cell.ID,
    pred_price = coalesce(price.x, price.y),
    pred_speed = coalesce(speed.x, speed.y),
    pred_hd = coalesce(hd.x, hd.y),
    pred_ram = coalesce(ram.x, ram.y),
    pred_screen = coalesce(screen.x, screen.y),
    pred_ads = coalesce(ads.x, ads.y)
  )


sorted_df <- merged_df
sorted_df$diff <- rowMeans(abs(sorted_df[, c("act_price","act_speed","act_hd","act_ram","act_screen","act_ads")] - sorted_df[, c("pred_price","pred_speed","pred_hd","pred_ram","pred_screen","pred_ads")]))
