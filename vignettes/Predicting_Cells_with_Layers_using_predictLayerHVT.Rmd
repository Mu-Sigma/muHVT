---
title: "muHVT: Predicting Cells with Layers using predictLayerHVT"
...
output:
  rmarkdown::html_vignette:
    ...
    toc_depth : 2
vignette: >
  %\VignetteIndexEntry{muHVT: Predicting Cells with Layers using predictLayerHVT}
...
df1 <- combined
df1 <- as.numeric(df1$Cell.ID)
df2 <- mapC_summary

aa <- df1 %>% 
  bind_rows(semi_join(df2, df1, by = "Cell.ID"))


# matched <- lapply(1:nrow(df2), function(x) {
#   lapply(1:nrow(df1), function(y) {
#     if (!is.na(df1$layer2[y]) && !is.na(df2$Cell.ID[x]) && df1$layer2[y] == df2$Cell.ID[x]) {
#       return(df2[x, ])
#     }
#   })
# })
...
# Assuming your first dataframe is named 'df1' and the column is named 'column1_df1'
# Assuming your second dataframe is named 'df2' and the column is named 'column1_df2'
# Replace 'df1', 'df2', 'column1_df1', and 'column1_df2' with your actual dataframe and column names

# Create an empty list to store the appended rows
appended_rows <- list()

# Iterate over each value in column1_df1 of df1
lapply(df1$Layer2.Cell.ID, function(value) {
  # Check if the value exists in column1_df2 of df2
  matching_rows <- df2[df2$Cell.ID == value, ]
  if (nrow(matching_rows) > 0) {
    # Append the matching rows to the list
    appended_rows <<- append(appended_rows, matching_rows)
  }
})

# Combine the appended rows into a single dataframe
appended_df <- do.call(rbind, appended_rows)

# Append the appended_df to df1
combined_df <- rbind(df1, appended_df)
