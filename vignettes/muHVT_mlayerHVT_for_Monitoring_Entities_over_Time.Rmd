...
This package additionally provides functionality to create a predictive set of maps for monitoring entities over time.

The creation of a predictive set involves three steps -
...
```{r mlayer_flow,echo=FALSE,warning=FALSE,fig.show='hold',message=FALSE,out.width='90%',fig.height=8,fig.cap='Figure 1: Flow diagram for creating a predictive set of maps using mlayerHVT()'}
...
DT::datatable(hvt_mapA[[3]]$compression_summary, class = 'cell-border stripe')
```
...
DT::datatable(cell_mapping_df, filter = 'top', options = list(pageLength = 10), class = 'cell-border stripe')
```
...
DT::datatable(hvt_mapB, filter = 'top', options = list(pageLength = 10), class = 'cell-border stripe')
```
...
DT::datatable(hvt_mapC[[3]]$compression_summary, class = 'cell-border stripe')
```
...
DT::datatable(cell_mapping_mapC, filter = 'top', options = list(pageLength = 10, autoWidth = TRUE), class = 'cell-border stripe')
```
...
The important parameters for the function `mlayerHVT` are as below
...
* __`hvt_mapB`__ - A list of hvt.results obtained from the HVT function : Map B
