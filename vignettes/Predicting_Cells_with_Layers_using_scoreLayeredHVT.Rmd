---
title: "HVT: Predicting Cells with Layers using scoreLayerHVT"
...
output:
  rmarkdown::html_vignette:
    ...
    toc_depth : 2
vignette: >
  %\VignetteIndexEntry{HVT: Predicting Cells with Layers using scoreLayerHVT}
...
We will pass the below mentioned model parameters along with training dataset (containing 5007 data points) to `trainHVT` function.
...
summaryTable(map_A[[3]]$summary,scroll = TRUE,limit = 500)
...
*  **`hvt.results`** - (2D/3D)  A list (hvt.result) obtained from the trainHVT function while performing hierarchical vector quantization on training data. This list provides an overview of the hierarchical vector quantized data, including diagnostics, tessellation details, Sammon's projection coordinates, and model input information.

* **`line.width`**	- (2D) A vector indicating the line widths of the tessellation boundaries for each layer.

* **`color.vec`**	- (2D) A vector indicating the colors of the tessellations boundaries at each layer.

* **`pch1`**	- (2D) Symbol, It plots the centroids with a particular symbol such as (solid circle, bullet, filled square, filled diamond) in the tessellations.(default = 21 i.e filled circle).

* **`centroid.size`**	- (2D) Size of centroids for each level of tessellations (default = 3).
...
* **`child.level`** - (2DHEATMAP/3D) A Number indicating the level for which the heat map is to be plotted.

* **`hmap.cols`** - (2DHEATMAP/3D) A Number or a Character which is the column number of column name from
...
* **`sepration_width`** - (3D) An integer indicating the width between two Levels.

* **`layer_opacity`** - (3D) A vector indicating the opacity of each layer/ level.

* **`dim_size`** - (3D) An integer indicating the dimension size used to create the matrix for the plot.

* **`heatmap`** -  A Character indicating which type of plot should be generated. Accepted entries are '2DHVT','2DHEATMAP', '3D'. Default value is 2DHVT.
...
The **`plotNovelCells`** function is used to plot the Voronoi tessellation using the compressed HVT map (map A) containing 440 cells and highlights the identified novelty cell(s) i.e 5 cells (containing 24 records) in red on the map.
...
plotNovelCells(identified_Novelty_cells, map_A,line.width = c(0.4),centroid.size = 0.01)
```

We pass the dataframe with novelty records (24 records) to trainHVT function along with other model parameters mentioned below to generate **map B** (layer2)
...
map_B <- trainHVT(dataset_with_novelty,
                  n_cells = 12 ,   #6
                  depth = 1,
                  quant.err = 0.2,
                  projection.scale = 10,
...
```{r, message=FALSE,warning=FALSE}
summaryTable(map_B[[3]]$summary,scroll = TRUE,limit = 500)
...
map_C <- list()
mapA_scale_summary = map_A[[3]]$scale_summary
map_C <- trainHVT(dataset_without_novelty,
                  n_cells = 10,
                  depth = 2,
                  quant.err = 0.2,
                  projection.scale = 10,
...
```{r,message=FALSE,warning=FALSE}
summaryTable(map_C[[3]]$summary,scroll = TRUE,limit = 500)
...
```{r sample test data computer,warning=FALSE,message=FALSE,eval = TRUE}
Table(head(testComputers_data))
```
...
-   We have considered computers dataset for creating a predictive sequence of maps using scoreLayeredHVT() in this vignette.

-   Our goal is to achieve data compression upto atleast `80%`.

-   We construct a compressed HVT map (Map A) using the trainHVT() on the training dataset by setting **`n_cells`** to 440 and **`quant.error`** to 0.2 and we were able to attain a compression of 81%.
...
-   The dataset with novelty is then passed to the trainHVT() to construct another HVT map (map B). But here, we set the parameters **`n_cells`** = 3, **`depth`** = 1 etc. when constructing the map.

-   The dataset without novelties is then passed to the trainHVT() to construct another HVT map (map C). But here, we set the parameters **`n_cells`** = 100, **`depth`** = 2 etc. when constructing the map.

-   Finally, the set of maps - map A, map B, and map C are passed to the scoreLayeredHVT() along with the test dataset to predict which map and what cell each test record is assigned to.
...
<!-- * From the output of `scoreLayeredHVT`we can see that 343rd observation from the test data having Cell.ID as A399 have been identified as novelty and is mapped to 1st(B1) novelty cell in Layer2.Cell.ID column .Similarly, 367th,423rd....... test record having Cell.ID as A399 is correctly identified as novelty and gets mapped to B1 novelty cell in the Layer2.Cell.ID column respectively. -->
